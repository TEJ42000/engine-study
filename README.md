...
- 🧱 **Start from [`v1-core/`](v1-core/)** — a verified, UI-agnostic TypeScript implementation of the data model **and the whole deterministic backbone**: two-axis maturity machine, study-next ordering, leak profile, export version gate, the localStorage storage layer, the `recordSession` write path (F3), course cascade-delete, editor/precision text helpers, and the v0→v1 migration. **Build the React UI on top of it; don't re-derive these rules.** It's isolated from the frozen prototype and its 36 checks pass (`cd v1-core && npm run check`).
